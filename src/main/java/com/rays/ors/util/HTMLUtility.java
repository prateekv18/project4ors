...
public class HTMLUtility {
	 ...
	 * 
	 * if(select){ sb.append("<option selected-value=''---->"); } for (String
	 * key : keys) { val = map.get(key); if (key.trim().equals(selectedVal)) {
	...
	public static String getList(String name, String selectedVal, HashMap<String, String> map) {
		...
		StringBuffer sb = new StringBuffer(
				"<select style='width: 208px;  height: 23px;' class='form-control' name='" + name + "'>");
	 ...
	 * 
	 * for (DropDownListBean obj : dd) { key = obj.getKey(); val =
	 * obj.getValue(); boolean select = true; if(select){
	 * sb.append("<option selected-value=''---->"); } if
	 * (key.trim().equals(selectedVal)) { sb.append("<option selected value='" +
	 * key + "'>" + val + "</option>"); } else { sb.append("<option value='" +
	 * key + "'>" + val + "</option>"); } } sb.append("</select>"); return
	 * sb.toString(); }
	 */
	public static String getList(String name, String selectedVal, List list) {
	...
	/*    *//**
			 ...
			 * @return
			 *//*
				 * public static String getSubmitButton(String label, boolean
				 * access, HttpServletRequest request) {
				 * 
				 * String button = "";
				 * 
				 * if (access) { button =
				 * "<input type='submit' name='operation'    value='" + label +
				 * "' >"; } return button; }
				 */
	/*
